...
(function(window,document,Laya){
	...
	*/
	...
	var Test=(function(){
		function Test(){
			...
			var arr=[];
			arr.push({url:"res/bg.png",type:"image"});
			arr.push({url:"res/yellow.png",type:"image"});
			Laya.loader.load(arr,Handler.create(this,this.loadImgComplete),null,"image");
		}
		...
		__proto.loadImgComplete=function(){
			...
			this.scroll.y=20;
			for (var i=0;i < 15;i++){
				var img=new Image("res/bg.png");
				this.scroll.addNode(img);
			}
			Laya.stage.on("click",this,this.clickHandler);
			Laya.stage.on("mousedown",this,this.stageMouseDownHandler);
			Laya.stage.on("mouseup",this,this.stageMouseUpHandler);
			Laya.stage.on("mouseout",this,this.stageMouseUpHandler);
			var img=new Image("res/bg.png");
			img.x=900
			img.y=20
		...
		__proto.stageMouseDownHandler=function(){
			this.label.text="stage mouse down";
		}

		__proto.stageMouseUpHandler=function(){
	...
	*@author ...Kanon
	*/
	//class components.ScorllContainer extends laya.display.Sprite
	var ScorllContainer=(function(_super){
		function ScorllContainer(){
			this.content=null;
			...
			ScorllContainer.__super.call(this);
			this.initData();
			this.initUI();
			this.initEvent();
		}

		__class(ScorllContainer,'components.ScorllContainer',_super);
		var __proto=ScorllContainer.prototype;
		/**
		*初始化数据
		*/
		...
		*/
		__proto.initEvent=function(){
			this.on("mousedown",this,this.contentMouseDownHandler);
			this.on("mouseup",this,this.contentMouseUpHandler);
			Laya.stage.on("mouseup",this,this.contentMouseUpHandler);
			Laya.stage.on("mouseout",this,this.contentMouseUpHandler);
		...
		*更新内容的显示大小
		*/
		__proto.updateContentSize=function(){
			var pos=0;
			for (var i=0;i < this.contentList.length;i++){
				var node=this.contentList[i];
		...
		*用于调试绘制内容的范围
		*/
		__proto.debugDrawContentBound=function(){
		...
		*@param index 索引
		*/
		__proto.removeNodeByIndex=function(index){
		...
		*删除所有子对象
		*/
		__proto.removeAllChild=function(){
		...
		*@param height 高度
		*/
		__proto.setViewSize=function(width,height){
		...
		*更新拖动速度
		*/
		__proto.updateTouchSpeed=function(){
			if (this.isTouched){
				if (!this._isHorizontal)
		...
		*是否横向滚动
		*/
		__getset(0,__proto,'isHorizontal',function(){return this._isHorizontal;},function(value){
		...
		*间隔
		*/
		__getset(0,__proto,'gap',function(){return this._gap;},function(value){
			this._gap=value;
		});
		...
		*设置是否有回弹
		*/
		__getset(0,__proto,'isBounce',function(){return this._isBounce;},function(value){
			this._isBounce=value;
		});

		return ScorllContainer;
	})(Sprite)
	...
	Laya.__init([Timer,LoaderManager,EventDispatcher,Render,Browser,LocalStorage]);
	new Test();
